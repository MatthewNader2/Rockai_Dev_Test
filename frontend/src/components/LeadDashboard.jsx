...
const StarIcon = ({ isPinned, ...props }) => (
  <svg {...props} xmlns="http://www.w.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
    ...
    >
      <Link to={`/leads/${lead.id}`} className="flex-grow">
        <h3 className="text-xl font-bold text-slate-100 truncate">
          {lead.nameEn && lead.nameEn.toLowerCase().trim() !== 'no name' ? lead.nameEn : `[Unnamed Lead]`}
        </h3>
  ...
  const processedLeads = useMemo(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(lead =>
        lead.nameEn?.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <KpiCard title="Total Leads" value={kpiData?.totalLeads} />
        <KpiCard title="Fresh Leads" value={kpiData?.freshLeads} />
        <KpiCard title="Open Deals" value={kpiData?.openDeals} />
        <KpiCard title="Conversion Rate" value={kpiData?.conversionRate} unit="%" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        <div className="lg:col-span-2 bg-slate-800 p-6 rounded-lg shadow-lg">
          <h3 className="text-xl font-semibold text-slate-200 mb-4">Pipeline Distribution</h3>
        ...
        <div className="bg-slate-800 p-6 rounded-lg shadow-lg">
          <h3 className="text-xl font-semibold text-slate-200 mb-4">Proactive AI Insight</h3>
          {globalPatterns?.strategicPatterns?.[0] ? (
            ...
            <p className="text-slate-400">AI is analyzing patterns...</p>
          )}
        </div>
      </div>

      <h2 className="text-3xl font-semibold mb-6 text-slate-300">Leads List</h2>
      ...
      <div className="bg-slate-800 p-4 rounded-lg mb-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 items-end">
        <div className="lg:col-span-2">
          <label htmlFor="search" className="block text-sm font-medium text-slate-400 mb-1">Search by Name</label>
          <input type="text" id="search" placeholder="e.g., Mohamed Hassan" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
        ...
        <div>
          ...
          <select id="status" value={filters.status} onChange={e => setFilters({...filters, status: e.target.value})}
            ...
            {filterOptions.statuses?.map(s => <option key={s} value={s}>{s.replace('_', ' ')}</option>)}
          </select>
        </div>
        <div>
        ...
        <div>
        <label htmlFor="interest" className="block text-sm font-medium text-slate-400 mb-1">Interest Level</label>
        <select id="interest" value={filters.interest} onChange={e => setFilters({...filters, interest: e.target.value})}
            ...
            {filterOptions.interests?.map(i => <option key={i} value={i} className="capitalize">{i}</option>)}
        </select>
        </div>

        <button onClick={handleReset} className="bg-slate-600 hover:bg-slate-500 text-white font-bold py-2 px-4 rounded-md w-full">Reset</button>
      </div>
